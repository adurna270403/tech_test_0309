...
def btc_trend_follow(
    ...
    `symbols` pins the traded crypto set; when omitted, every USDT quote in the
    panel qualifies -- which silently re-sizes the sleeve whenever the panel
    gains columns, so production callers pass it explicitly.
    """
    px = close.ffill()
    crypto = [c for c in px.columns if c.endswith("USDT")] if symbols is None \
        else [s for s in symbols if s in px.columns]
