...
# Headline window starts 2017: that is when crypto history begins, so it is the
# first date at which all sleeves are live. Pre-2017 equity history is run
# separately as an equity-only robustness check, not discarded.
START = "2017-01-01"
IS_START = "2017-01-01"
IS_END = "2022-12-31"
...
EQ_OFF_WEIGHT = 0.25        # residual gross when it is not
EQ_HYSTERESIS = 0.01
...
# gross-capped book. output/hitrate_frontier.csv sweeps the grid.
SLEEVE_BUDGETS = {
    "equity_core": 0.57,
    "btc_trend": 0.25,
    "defensive": 0.18,
