...
from signals.carry import carry_pair_exposure, route_through_perp
...
def test_carry_gate_invariant_to_future_data():
    """The carry gate reads trailing funding only; appending future funding
    must not change any past gate value."""
    idx = pd.bdate_range("2020-01-01", periods=400)
    rng = np.random.default_rng(13)
    funding = pd.DataFrame({"XUSDT": rng.normal(0.0002, 0.0005, len(idx))}, index=idx)
    base = carry_pair_exposure(funding, lookback=21)
    future_idx = pd.bdate_range(idx[-1] + pd.Timedelta(days=1), periods=60)
    ext = carry_pair_exposure(
        pd.concat([funding, pd.DataFrame(
            {"XUSDT": rng.normal(-0.001, 0.0005, 60)}, index=future_idx)]),
        lookback=21)
    pd.testing.assert_frame_equal(
        base.fillna(0), ext.iloc[:len(base)].fillna(0), check_exact=False, rtol=1e-12)


def test_carry_gate_needs_full_lookback():
    """A gate that fires before it has a full trailing window of funding would
    trade on partial history."""
    idx = pd.bdate_range("2020-01-01", periods=30)
    funding = pd.DataFrame({"XUSDT": 0.0003}, index=idx)
    expo = carry_pair_exposure(funding, lookback=21)
    assert expo["XUSDT"].iloc[:20].abs().max() == 0.0
    assert expo["XUSDT"].iloc[20] == 1.0


def test_perp_routing_keeps_total_exposure_and_uses_trailing_data():
    """Routing moves a position between spot and perp legs: total exposure per
    asset must be unchanged, and the decision must depend only on funding
    already published (perturbing FUTURE funding leaves past routing intact)."""
    idx = pd.bdate_range("2020-01-01", periods=300)
    rng = np.random.default_rng(17)
    funding = pd.DataFrame({"XUSDT": rng.normal(0.0002, 0.0006, len(idx))}, index=idx)
    expo = pd.DataFrame({"XUSDT": -0.4, "XUSDT-PERP": 0.0}, index=idx)

    routed = route_through_perp(expo, funding, lookback=21)
    total_before = expo.sum(axis=1)
    total_after = routed.sum(axis=1)
    pd.testing.assert_series_equal(total_before, total_after)

    future = pd.concat([funding, pd.DataFrame(
        {"XUSDT": rng.normal(-0.002, 0.0006, 60)},
        index=pd.bdate_range(idx[-1] + pd.Timedelta(days=1), periods=60))])
    routed_ext = route_through_perp(expo, future, lookback=21)
    pd.testing.assert_frame_equal(
        routed, routed_ext.iloc[:len(routed)], check_exact=False, rtol=1e-12)
...
def test_wd_mom_uses_only_completed_weeks():
    """The weekly resample at W-FRI must be right-labelled: perturbing FUTURE
    prices leaves past signals unchanged, and the signal is flat within the
    week (formed once at Friday close)."""
    rng = np.random.default_rng(23)
    idx = pd.date_range("2020-01-01", periods=200, freq="D")
    prices = pd.DataFrame(
        100 * np.cumprod(1 + rng.normal(0, 0.02, size=(len(idx), 4)), axis=0),
        index=idx, columns=["AUSDT", "BUSDT", "CUSDT", "DUSDT"])
    base = wd_momentum(prices, k=4, n_long=2)

    future_idx = pd.date_range(idx[-1] + pd.Timedelta(days=1), periods=60, freq="D")
    ext = wd_momentum(pd.concat([prices, pd.DataFrame(
        100 * np.cumprod(1 + rng.normal(0, 0.02, size=(60, 4)), axis=0),
        index=future_idx, columns=prices.columns)]), k=4, n_long=2)
    pd.testing.assert_frame_equal(base, ext.iloc[:len(base)], check_exact=False, rtol=1e-12)

    # flat within week: weights only change on Friday (the W-FRI resample)
    held = base[(base.abs().sum(axis=1) > 0)]
    changed = held[held.diff().abs().sum(axis=1) > 0]
    assert len(held) > 0
    assert all(d.dayofweek == 4 for d in changed.index), (
        f"weights changed off-Friday on: {[str(d.date()) for d in changed.index[:5]]}")


def test_wd_mom_no_exposure_before_full_history():
    """k-week momentum needs k+1 weekly observations; nothing before that."""
    idx = pd.date_range("2020-01-01", periods=35, freq="D")  # ~5 weeks
    prices = pd.DataFrame(100.0, index=idx, columns=["AUSDT", "BUSDT"])
    expo = wd_momentum(prices, k=4, n_long=1)
    assert expo.abs().max().max() == 0.0


def test_wd_mom_liquidity_filter_is_point_in_time():
    """The ADV liquidity filter must use trailing volume only: perturbing
    future volume cannot change past eligibility."""
    from signals.wd_mom import liquidity_mask
    rng = np.random.default_rng(31)
    idx = pd.date_range("2020-01-01", periods=120, freq="D")
    close = pd.DataFrame(
        100 * np.cumprod(1 + rng.normal(0, 0.02, size=(len(idx), 3)), axis=0),
        index=idx, columns=["AUSDT", "BUSDT", "CUSDT"])
    vol = pd.DataFrame(np.abs(rng.normal(1e6, 2e5, size=(len(idx), 3))), index=idx,
                       columns=close.columns)
    base = liquidity_mask(vol, close, window=20, n_liquid=2)
    future_idx = pd.date_range(idx[-1] + pd.Timedelta(days=1), periods=40, freq="D")
    vol_ext = pd.concat([vol, pd.DataFrame(
        np.abs(rng.normal(1e9, 1e5, size=(40, 3))), index=future_idx,
        columns=vol.columns)])
    close_ext = pd.concat([close, close.tail(1).pipe(
        lambda c: pd.DataFrame(np.repeat(c.values, 40, axis=0),
                               index=future_idx, columns=c.columns))])
    ext = liquidity_mask(vol_ext, close_ext, window=20, n_liquid=2)
    pd.testing.assert_frame_equal(base, ext.iloc[:len(base)], check_exact=False, rtol=1e-12)
