...
Produces:
  ...
  hitrate_frontier.csv     defensive x crypto budget grid at the 75% bar
  budget_frontier.csv      avg month vs hit rate as the crypto budget moves
  risk_ablation.csv        leave-one-out on the risk overlays
  cost_sensitivity.csv     commission / slippage grid
  execution_sensitivity.csv  same-close vs next-open vs next-close fills
  cross_asset_experiment.csv  managed-futures book across 29 ETFs (rejected)
  candidate_alphas.csv     externally proposed alphas (rejected)
"""
import json
import sys
...
WINDOWS = {
    "is_window": (config.IS_START, config.IS_END),
...
def _btc_kwargs():
    from data.loaders import CRYPTO_SYMBOLS
    return dict(
...
def sensitivity_grids(bt: Backtester, close: pd.DataFrame, eps: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "equity_core/off_weight": {n: eq(off_weight=float(n)) for n in [0.0, 0.05, 0.15, 0.25, 0.40]},
        ...
        for dw in [0.10, 0.12, 0.15, 0.18]:
            for cw in [0.20, 0.25, 0.30, 0.35, 0.40]:
                ...
                row["meets_75_all_windows"] = bool(
                    ...
                    and (monthly_returns(_slice(r, "is_window")) > 0).mean()
...
def main():
    ...
    close = close_frame(data)
    eps = load_eps()
    sleeves = build_sleeves(close, eps, data_volumes={s: d["volume"] for s, d in data.items()})
    bt = Backtester(data)
    ...
    sr_trials = np.array([
        sharpe(_slice(sn["btc_trend"], "is_window")),
        sharpe(_slice(sn["equity_core"], "is_window")),
        sharpe(_slice(sn["wq_earnings"], "is_window")),
    ...
    for c in sn:
        ...
        row["decay_ratio"] = (row["oos_2023_plus"] / row["is_window"]
                              if row["is_window"] else np.nan)
        decay[c] = row
    ...
    for r in hr:
        print(f"  {r['defensive_budget']:5.2f} {r['crypto_budget']:5.2f} "
              f"{r['full/pct_pos']*100:8.1f}% {r['is_window/pct_pos']*100:6.1f}% "
