...
import config
from backtest import Backtester, monthly_returns, tbill_rf_series
from data.loaders import (CRYPTO_SYMBOLS, CRYPTO_XS_SYMBOLS,
...
def carry_overlay(close: pd.DataFrame) -> pd.DataFrame:
    funding = load_funding()
    if funding.empty:
        return pd.DataFrame(0.0, index=close.index, columns=close.columns)
    expo = carry_sleeve_exposure(close, funding, config.CARRY_PAIRS,
                                 lookback=config.CARRY_LOOKBACK)
...
def build_sleeves(close: pd.DataFrame, eps: pd.DataFrame,
    ...
    otherwise still enter the risk-budget normalisation. `data_volumes` (symbol
    -> daily volume series) is required for the wd_mom liquidity filter.
    """
    all_sleeves = {
        ...
        "wd_mom": wd_momentum(
            close[CRYPTO_XS_SYMBOLS],
            k=config.WD_K_WEEKS,
            n_long=config.WD_N_LONG,
            volume=pd.DataFrame(
...
def main():
    ...
    rf_pre = tbill_rf_series(close.index) if config.USE_TBILL_CASH_YIELD else None
    bt_pre = Backtester(data, start=config.EQ_ROBUSTNESS_START,
                        end=config.EQ_ROBUSTNESS_END, rf=rf_pre)
    ...
    summary = {
        ...
        "is_window": metrics(out.loc[is_mask, "ret_net"]),
    ...
    for k in ["full", "equity_only_2010_2016", "is_window", "oos_2023_plus"]:
        mm = summary[k]
        print(f"\n== {k} (net) ==")
