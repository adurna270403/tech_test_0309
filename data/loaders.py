...
# Extended cross-section for the same-weekday momentum sleeve: every Binance
# USDT pair listed before 2020 with usable history (delisted names included,
# so the panel is not survivorship-filtered). The sleeve trades only the
# top-12 by trailing dollar volume each day, so illiquid names are priced in
# only when they are actually liquid.
CRYPTO_XS_SYMBOLS = CRYPTO_SYMBOLS + [
    "NEOUSDT", "QTUMUSDT", "EOSUSDT", "XLMUSDT", "IOTAUSDT", "TRXUSDT",
    "ETCUSDT", "ICXUSDT", "VETUSDT", "WAVESUSDT", "ZILUSDT", "XMRUSDT",
    "ZECUSDT", "DASHUSDT", "OMGUSDT", "NANOUSDT", "ATOMUSDT", "ALGOUSDT",
    "BCHUSDT",
]
...
def load_tbill() -> pd.DataFrame:
    """13-week T-bill discount rate in PERCENT (^IRX daily close)."""
    path = SNAP / "tbill_irx.parquet"
    if not path.exists():
        raise FileNotFoundError(f"Missing {path}; run scripts/fetch_funding.py")
    return _read_symbol("tbill_irx")
...
def load_all(include_xassets: bool = False) -> dict[str, pd.DataFrame]:
    ...
    + the extended crypto cross-section (wd_mom sleeve) + crypto perps
    (carry sleeve legs).
    ...
    rejected (see run_validation.cross_asset_experiment and the report's
    diversifiers section). They stay in the snapshot so that result stays
    reproducible.
    """
    out = {}
    ...
    for s in CRYPTO_XS_SYMBOLS:
        if s not in out and (SNAP / f"{s}.parquet").exists():
            out[s] = _read_symbol(s)
    out.update(load_perps())
    return out
